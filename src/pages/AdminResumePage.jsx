...
function AdminResumePage() {
    ...
    const [targetKeywords, setTargetKeywords] = useState('React.js, Node.js, MongoDB Atlas, Docker, AWS');
    ...
    useEffect(() => {
        const fetchExisting = async () => {
            if (currentUser?.username) {
                try {
                    const res = await resumeBuilderSurvice.ReadResume(currentUser.username);
    ...
    const handleGenerate = async (e) => {
        ...
        try {
            const res = await resumeBuilderSurvice.CreateAiResume(title, keywordsArray, customLinks);
            if (res?.data) {
    ...
    return (
        <div className="max-w-6xl mx-auto px-4 py-8 space-y-8">
            <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
                <h1 className="text-2xl font-bold text-slate-900 mb-2">AI Resume Generator</h1>
                <p className="text-slate-600 text-sm mb-6">Customize target keywords and generate an ATS-optimized resume directly from your database contents.</p>

                {message && (
                    <div className={`p-4 mb-4 text-sm font-bold rounded-lg ${message.includes('Failed') ? 'bg-red-50 text-red-700' : 'bg-green-50 text-green-700'}`}>
                        {message}
                    </div>
                )}
                ...
                <form onSubmit={handleGenerate} className="space-y-4">
                    <div>
                        <label className="block text-sm font-bold text-slate-700 mb-1">Target Title</label>
                        <input 
                            type="text" 
                            value={title} 
                    ...
                    <div>
                        <label className="block text-sm font-bold text-slate-700 mb-1">Target Keywords (Comma Separated)</label>
                        <input 
                            type="text" 
            ...
            {currentResume && (
                <div className="border-t pt-8">
                    <h2 className="text-xl font-bold text-slate-900 mb-4 text-center">Live Preview & Print</h2>
